...
module Value =
    struct
        ...
        (* A function fromInt with a modulo 13 and toInt could be used to prettify the
         * next two functions but we must respect a norm.
        *)
...
let isSpade card color      = isOf card Color.Spade

let isHeart card color      = isOf card Color.Heart

let isDiamond card color    = isOf card Color.Diamond

let isClub card color       = isOf card Color.Club
(* -------------------------------------------------------------------------- *)
